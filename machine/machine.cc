...
Machine::Machine(bool debug)
{
    ...
    vmfile = new char*[MaxThread];
    vmbitmap = new BitMap(MaxThread);
    vm_idx = -1;
...
/***** LAB 4 BEGIN *****/
int Machine::allocPhysPage()
{
    int idx = bitmap->Find();
#ifdef PHYSPAGE_ALLOC_TEST
    if (idx == -1)
        printf("Out of physical page.\n", idx);
    else
...
int Machine::freePhysPage()
{
    for (int i = 0; i < pageTableSize; i++)
    {
        if (pageTable[i].valid)
        {
        int curr = pageTable[i].physicalPage;
        if (bitmap->Test(curr))
        {
            bitmap->Clear(curr);
#ifdef PHYSPAGE_ALLOC_TEST
...
int Machine::allocVM()
{
    return vmbitmap->Find();
}

int Machine::freeVM(int idx)
{
    if (vmbitmap->Test(idx))
    {
        vmbitmap->Clear(idx);
        return 0;
    }
